...
object DiffablePrinterListsAndBuilderWithSizing {
  ...
  case class HintState(indent: Int, accumulated: Int)

  def go(events: List[PrinterAction]) = {
    val sizeHint = events.foldLeft(HintState(0, 0)) { (a, action) =>
      action match {
        case Indent => a.copy(indent = a.indent + 1)
        case Unindent => a.copy(indent = a.indent - 1)
        case Print(s) => a.copy(accumulated = a.accumulated + s.length)
        case Newline => a.copy(accumulated = a.accumulated + 1 + (2*a.indent))
        case Trim => a.copy(accumulated = a.accumulated - 1 - (2*a.indent))
      }
    }.accumulated
    events.foldLeft(new Interpreter(buffer = new StringBuilder(sizeHint))) {
